...
| Working project | In progress | `uv run codescope version`; CLI index/status/search/reset acceptance; Phase 1–7 tests | Complete MCP, demonstration, and final P0 gates | End-to-end local indexing, semantic CLI search, symbol lookup, similar-code evidence, status validation, and exact-runtime reset work; MCP behavior is not implemented. |
| Codex and GPT-5.6 usage | In progress | README collaboration section; Codex thread history; Phase 5–7 security ledgers | Preserve the primary implementation-session evidence | Phase 1–7 implementation, validation, offline-model integration, and security-review usage is documented; final demo usage is pending. |
| Dated Build Week commit evidence | In progress | `git log --oneline --decorate`; `BUILD_WEEK_CHANGELOG.md` | Continue phase-scoped dated commits | Phase 0–6 commits exist on `main`; Phase 7 is complete only in the working tree and awaits owner commit authorization. |
...
| README setup and testing instructions | In progress | `README.md`; Phase 7 isolated CLI acceptance | Expand and clean-clone test the final MCP quickstart | Setup, model policy, testing, sample indexing, semantic search, JSON, and reset are documented; final MCP judge instructions remain pending. |
| Codex acceleration narrative | In progress | README collaboration section; `BUILD_WEEK_CHANGELOG.md` | Add concrete later-phase examples and owner edits | Phase 1–7 research, implementation, deterministic ranking, CLI integration, terminal hardening, offline-model validation, and security-review examples are recorded without final submission claims. |
...
| Installation instructions | In progress | `README.md` setup and current-operation sections | Clean-clone verify model preparation and final MCP operation | Locked environment and Phase 7 CLI acceptance behavior are documented; clean-clone and final MCP setup remain pending. |
...
| Direct judge testing path | In progress | `README.md` current operation; `tests/fixtures/sample_python/`; Phase 7 CLI integration | Add final MCP steps and clean-clone timing | Deterministic index/status/search/JSON/reset commands work offline with the prepared model, but this is explicitly not the final judge workflow. |
| Sample repository or sample data | In progress | `tests/fixtures/sample_python/`; Phase 5–7 integration tests | Polish and license-review the final judge sample | The fixture is deterministically indexable and the offline CLI finds `validate_email` at the expected relative path and lines; final judge presentation is pending. |
| Final security checks | In progress | Phase security reports; 75-pass/1-platform-skip security suite; `BUILD_WEEK_CHANGELOG.md` | Repeat security checks after later implementation phases and before submission | Phase 7 reviewed the full CLI/reset diff, corrected two reproduced terminal-integrity defects, and completed post-fix review with no remaining candidate. The final MVP review remains future work. |
